...
Q1_tilde = (1./re).*h.^2.*(1-h).*(h.*(m-1)+1).*((rho-1).*G_param.*(1./So).*hx + epsilon.^2.*we.*real(datahLx)) + (1./120).*h.*a0.*(h.*a1x-2.*h.*hx.*a1).*(13.*h.^2.*(m-1)-10.*m.*h+26.*h-13) - (3./(40.*h.*m)).*rho.*(h-1).*(3.*h.^2.*(m-1)+4.*h-1).*(m.*h.*b0x.*(m.*h.*b1+a0.*(h-1))-hx.*a0.^2.*(h-1)) - (1./840).*h.*hx.*(a1.^2).*(33.*h.^2.*(m-1)-34.*m.*h+66.*h-33) + (1./840).*rho.*h.*hx.*(b1.^2).*(33.*h.^2.*(m-1)+32.*h+1) + (3./40).*h.*(h.*a0x.*a0+h.*a0x.*a1-hx.*a0.^2).*(3.*h.^2.*(m-1)-2.*m.*h+6.*h-3) + (1./(60.*m)).*rho.*(h-1).*a0.*(13.*h.^2.*(m-1)+16.*h-3).*(hx.*b1-(1./2).*(h-1).*b1x) - (1./840).*rho.*h.*(h-1).*(111.*h.^2.*b1.*(m-1)+142.*b1h-31.*b1).*b1x + (1./840).*h.^2.*(a1.*a1x).*(111.*h.^2.*(m-1)+222.*h-80.*m.*h-111) + qx.*((-1./40).*h.*rho.*(7.*h.^2.*(m-1)+6.*h+1).*b1+(1./20).*h.*rho.*(h-1).*b1h.*(8.*h.^2.*(m-1)+9.*h-1)-(1./20).*h.^2.*a1h.*(8.*h.^2.*(m-1)+16.*h-7.*m.*h-8)+(1./40).*h.*a1.*(7.*h.^2.*(m-1)-8.*m.*h+14.*h-7)+(1./(8.*m).*rho.*(h-1).*(5.*h.^2.*(m-1)+6.*h-1).*(m.*h.*b0h-a0))-(1./8).*h.*(5.*h.^2.*(m-1)-4.*m.*h+10.*h-5).*(a0h.*h-a0));

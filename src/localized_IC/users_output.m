...
if handles.saveoptions.vtk ==1
    ...
    if parameter.Scaling==2 %Nusselt scaling
        ...
        if sizeU(2) == M2*(N1+1)*7 %Full second-order model
            combined_mat = [mod_x_coord, flipud(real(datah(:,1)))*ScaleH, flipud(real(dataq(:,1)))*ScaleQ, flipud(real(dataS1(:,1)))*(ScaleQ/ScaleH), flipud(real(dataS2(:,1)))*(ScaleQ/ScaleH)];
        else

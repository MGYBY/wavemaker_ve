...
sizeU=size(U(end,:));
num_wx=parameter.stNwaves;

%make all the output for the user
if sizeU(2) == M2*(N1+1)*7 %Full second-order model
    % [datah,dataq,dataS1,dataS2,~]   = reconstruct_U_full(U,N1,N2,M1,M2);
    [datah, dataq, datap, dataS1, dataS2, dataR1, dataR2]   = reconstruct_U_full(U,N1,N2,M1,M2);
else
...
elseif parameter.Scaling==2 %Nusselt scaling
    ...
    % ScaleX = 2*pi()/parameter.Nusselt_k0x*num_wx;
    % FIXME: keep ScaleZ unchanged for now, 2D has no meaning
    ScaleZ = 2*pi()/parameter.Nusselt_k0z;
    ScaleH = 1;
    ScaleQ = 1;
...
cla(handles.axes2,'reset')
    ...
    % account for localized dist (plotting needs to be modified as well!)
    % axis(handles.axes2,[0 1 0 max(storeData.hmax)*1.1]);
    axis(handles.axes2,[0 1*num_wx 0 max(storeData.hmax)*1.1]);
    ...
    contour(handles.axes2,(0:1/(M1-1):1)*ScaleX,y*ScaleH,fliplr(-psi),28);
...
if handles.saveoptions.vtk ==1
    ...
    filename_zip = ([parameter.Name '/' sprintf('%06d',round(ttime*ScaleT,3)) 'Velocity_field']);
    zip(filename_zip, filename);
    % delete(stract("./", filename));
    delete(filename);
    
    %%% VTK also means text output
    if parameter.Scaling==2 %Nusselt scaling
        % mod_x_coord = transpose(0:1*(2*pi()/parameter.Nusselt_k0x)/(M1-1):1*2*pi()/parameter.Nusselt_k0x);
        % mod_x_coord = reshape(x,nr_of_elements,1);
        % mod_y_coord = reshape(y,nr_of_elements,1);
        if sizeU(2) == M2*(N1+1)*7 %Full second-order model
            combined_mat = zeros(M1*M2, 9);
            for m=[1:M1]
                x=(m-1)*Lx/M1;
                for n=[1:M2]
                    z=(n-1)*Lz/M2;
                    ind = (m-1)*M2+n;
                    combined_mat(ind, 1) = x;
                    combined_mat(ind, 2) = z;
                    combined_mat(ind, 3) = datah(m,n)*ScaleH;
                    combined_mat(ind, 4) = dataq(m,n)*ScaleQ;
                    combined_mat(ind, 5) = datap(m,n)*ScaleQ;
                    combined_mat(ind, 6) = dataS1(m,n)*(ScaleQ/ScaleH);
                    combined_mat(ind, 7) = dataS2(m,n)*(ScaleQ/ScaleH);
                    combined_mat(ind, 8) = dataR1(m,n)*(ScaleQ/ScaleH);
                    combined_mat(ind, 9) = dataR2(m,n)*(ScaleQ/ScaleH);
                end
            end
        else % things should not happen here for now, since using Nusselt scaling
            combined_mat = zeros(M1*M2, 5);
            for m=[1:M1]
                x=(m-1)*Lx/M1;
                for n=[1:M2]
                    z=(n-1)*Lz/M2;
    ...
    filename_text = ([parameter.Name '/' sprintf('%06d',round(ttime*ScaleT,3)) 'text_field.csv']);
    csvwrite (filename_text, combined_mat);

    % combined_mat = [0:1/(M1-1):1)*ScaleX, flipud(real(datah(:,1)))*ScaleH, flipud(real(dataq(:,1)))*ScaleQ];
    % mod_x_coord = transpose(0:1/(M1-1):1*ScaleX);
    % if parameter.Scaling==2 %Nusselt scaling
    %     % mod_x_coord = transpose(0:1*(2*pi()/parameter.Nusselt_k0x)/(M1-1):1*2*pi()/parameter.Nusselt_k0x);
    %     mod_x_coord = reshape(x,nr_of_elements,1);
    %     mod_y_coord = reshape(y,nr_of_elements,1);
    %     if sizeU(2) == M2*(N1+1)*7 %Full second-order model
    %         combined_mat = [mod_x_coord, mod_y_coord, flipud(real(datah(:,1)))*ScaleH, flipud(real(dataq(:,1)))*ScaleQ, flipud(real(dataS1(:,1)))*(ScaleQ/ScaleH), flipud(real(dataS2(:,1)))*(ScaleQ/ScaleH)];
    %     else
    %     combined_mat = [mod_x_coord, mod_y_coord, flipud(real(datah(:,1)))*ScaleH, flipud(real(dataq(:,1)))*ScaleQ];
    %     end
    % else % things should not happen here for now, since using Nusselt scaling
    %     mod_x_coord = transpose(0:1/(M1-1):1);
    %     combined_mat = [mod_x_coord];
    % end
    % filename_text = ([parameter.Name '/' sprintf('%06d',round(ttime*ScaleT,3)) 'text_field.csv']);
    % csvwrite (filename_text, combined_mat);

    % output hmax
    dlmwrite(([parameter.Name '/' 'hmax-t.txt']),[ttime*ScaleT hmax],'delimiter',' ','-append')

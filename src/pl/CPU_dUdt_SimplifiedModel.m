...
n_index = 0.607;
Fn = (11.*n_index+6.0)./(4.*n_index+3.0);
Gn = (6.*n_index+3.0)./(4.*n_index+3.0);
In = (3.*n_index+2.0)./(2.*(2.*n_index+1.0));
K0n = -1.*(n_index-1.0).*(2.*n_index+1.0).*(3.*n_index+2.0)/((n_index+1.0).*(n_index+1.0));
J0n = K0n;
L0n = K0n./2.0;
M0n = -1.0.*(n_index-1.0).*(3.*n_index+2.0)./(2.*(n_index+1.0));
phi0n = n_index./(2.*n_index+1.0);
Jwn = (3.*n_index.*n_index+13.*n_index+8.0)./((n_index+2.0).*(n_index+1.0));
Kwn = (5.*n_index+4.0)./(n_index+1.0);
Mwn = Kwn;
Lwn = (17.*n_index.*n_index+23.*n_index+8.0)./(2.*((n_index+1.0).*(n_index+1.0)));

% power-law dimless parameters %
re_num = 50.0;
inc_num = 3.7320508;
% we_num = 27.39;
we_num = 0.0;
...
h3=h2.*h;
% power-law consideration here %
rhq = (s_val.^2.0 + (2.*(2.*n_index+1.0)./(n_index+1.0).*(-1.*q.*hx./(h.^2.0)+qx./h)).^2.0 + ((2.*n_index+1.0)./(n_index+1.0).*(-2.*hx.*qx./h+q.*(2.*hx.^2.0./h.^2.0-hxx./h)+qxx)).^2.0).^((n_index-1.0)./2.0);
...
% power-law consideration here %
dqdt = (-1.*Fn.*q./h.*qx + Gn.*(q.^2.0./(h.^2.0)).*hx) + In./re_num.*(h.*(1.0-inc_num.*hx+we_num.*real(datahLx))-(q.*(abs(q)).^(n_index-1.0))./((phi0n.*h.^2.0).^n_index)) + ...
                  1./re_num.*rhq.*(J0n.*q./(h.^2.0).*((hx).^2.0) - K0n.*qx.*hx./h - L0n.*q./h.*hxx + M0n.*qxx) + ...
                  1./re_num.*h.^((n_index-1.0)./n_index).*(Jwn.*q./(h.^2.0).*(hx).^2.0 - Kwn.*qx.*hx./h - Lwn.*q./h.*hxx + Mwn.*qxx);

function [dUdt] = CPU_dUdt_SimplifiedModel(yU,h0,M1,M2,N1,N2,kx,kz,delta,eta,zeta,k2cut,N,A,hs,mparam,rey,cot,we)
...
%%% check if viscoelastic fluids or not
% if (r_vis == 0 | deb_num == 0)
% dqdt = (((9./7).*q.*q./(h.*h) - (5.*cot_theta./2)./re_num.*h).*hx - (18.0./7.0)./h.*q.*qx + q./(7.*h).*qx + ...
...
dqdt = (((9./7).*q.*q./(h.*h) - (5.*cot_theta./2)./re_num.*h).*hx - (18.0./7.0)./h.*q.*qx + q./(7.*h).*qx + ...
                  (5./(2.*delta_num*re_num).*(h - q./h2)) + (5.*delta_num.*delta_num.*weber_num)./(2.*re_num).*h.*real(datahLx) + ...
                  delta_num./re_num.*(9./2.*qxx - 9./2./h.*qx.*hx + 4.*q./h2.*hx.*hx - 6.*q./h.*hxx) + ...
                  (5.*m_val)./(2.*re_num).*(6.*q.*qx./h3 - 6.*q.*q.*hx./(h2.*h2)))./(1 - (5.*m_val)./(2.*re_num.*h2));
% else
% end
